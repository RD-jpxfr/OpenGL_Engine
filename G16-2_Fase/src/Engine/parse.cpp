...
// Função que carrega as coordenadas dos ficheiros .3d para memória
vector< tuple<float, float, float> > readfile(string filename) {
...
vector<Model> parseModels(XMLElement* fnode) {
    ...
    if (temp) {
        ...
        vector< tuple<float, float, float> > cords;
...
void parseGroup(XMLElement* fnode, vector<Group>& groups){
    ...
    while (fnode) {
        ...
        groups.push_back(Group(t, r, s, models, subGroups, order));
